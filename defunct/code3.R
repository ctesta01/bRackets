...
build_bracket <- function(
  ...
  if (!is.null(segments)) {
    ...
    if (orientation == "v") {
      ...
      # Label placement:
      # - If bracket is on the LEFT: label should be to the LEFT
      # - If bracket is on the RIGHT: label should be to the RIGHT
      if (!is.null(text)) {
        if (is.null(label_xy)) {
          # Note: For vertical brackets, side indicates where the bracket sits relative to content.
          # We use that as the semantic "left-hand-side" or "right-hand-side" bracket.
          if (side == "left") {
            x_text <- x_br - tick - text_nudge
            hjust  <- 1
          } else {
            x_text <- x_br + tick + text_nudge
            hjust  <- 0
    ...
    if (orientation == "h") {
      ...
      # Label placement:
      # - ticks going UP: label BELOW
      # - ticks going DOWN: label ABOVE
      if (!is.null(text)) {
        if (is.null(label_xy)) {
          x_text <- (xl + xr) / 2
          hjust  <- 0.5
...
bracket_specs <- list(
  # Week 1: High vs Control (left-side bracket: ticks point right)
  list(
    orientation = "v",
    x0 = x_W1 - 0.22,          # nudge left of W1
    y0 = y_ctrl_W1,
    y1 = y_high_W1,
    side = "left",            # bracket sits on the left; ticks point left
    tick = 0.10,
    pad = 0.05,
    text = "***  d = 1.65",
    color = "black",
  ...
  # Week 1: Control vs Low (left-side bracket)
  list(
    orientation = "v",
    x0 = x_W1 - 0.34,
    y0 = y_low_W1,
  ...
  # High: W1 -> W2 (top dashed; ticks DOWN, so label ABOVE)
  list(
    orientation = "h",
    x0 = x_W1, x1 = x_W2,
    y0 = y_high_top + .5,
  ...
  # Low: W1 -> W2 (bottom dashed; ticks UP, so label BELOW)
  list(
    orientation = "h",
    x0 = x_W1, x1 = x_W2,
    y0 = y_low_bot - .5,
